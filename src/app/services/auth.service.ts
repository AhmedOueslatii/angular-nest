...
changeProjectStatus(projectId: number, newStatus: string): Observable<any> {
  const httpOptions = {
    headers: new HttpHeaders({
      'Content-Type': 'application/json',
    }),
  ...
  return this.http.patch<any>(`${this.baseUrl}/project/${projectId}/status`, statusChangeData, httpOptions);
}
